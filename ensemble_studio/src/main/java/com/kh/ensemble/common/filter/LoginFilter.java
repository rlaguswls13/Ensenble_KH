...
@WebFilter(filterName = "loginFilter", urlPatterns = {"/member/myPage", "/member/updateMember", "/member/changePwd", "/reservation/*" })
public class LoginFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// 회원 전용 서비스 요청이 왔을 때
		// session 에 loginMember가 있는지 확인하여
		// 없으면 메인페이지로 돌려보내기
		...
		if(session.getAttribute("loginMember")==null) {
			...
			//4. null == 로그인 x ==> 메인페이지로 강제 이동 == 메인페이지 재요청
